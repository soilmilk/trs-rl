#!/usr/bin/env python3
"""
scripts/evaluate_amc.py

Evaluate a checkpoint (or base model) on AMC (AI-MO/aimo-validation-amc).
...
SYSTEM = (
    "You are a careful mathematician. Solve the following AMC problem step by step. "
    "Show your reasoning, then put your final integer answer "
    "inside \\boxed{}."
)
...
def evaluate(
    ...
    if not problems:
        raise RuntimeError(
            ...
            "Did you run data/generate_amc.py?"
        )
    ...
    print("\n" + "="*50)
    print(f"  AMC Evaluation Results")
    print("="*50)
...
def main():
    ...
    p.add_argument("--eval-file",   required=True, help="Path to AMC JSONL")
    p.add_argument("--model",       default="/workspace/models/Qwen3.5-2B")
    p.add_argument("--n-samples",   type=int, default=83)
